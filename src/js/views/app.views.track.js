...
var Track = ItemView.extend({
  ...
  serializeData: function() {
    ...
    data.gain = Utils.scale(Math.sqrt(data.gain), 0, 1.15, 220, 0);
    data.pan = Utils.scale(data.pan, -1, 1, -150, 150);
  ...
  dragFader: function( ev, max ) {
    ...
    this.model.set('gain', Math.pow(Utils.scale(css, 0, 220, 1.15, 0), 2));
  },
  ...
  dragPanner: function( ev ) {
    ...
    if ( deg >= -150 && deg <= 150 ) {
      ...
      this.model.set('pan', Utils.scale(deg, -150, 150, -1, 1));
    }
  },
  ...
  resetFader: function() {
    var top = Utils.scale(1, 0, 1.15, 220, 0);
  ...
  drawMeter: function() {
    if ( typeof this.ui.canvas !== 'string' ) {
      ...
      var canvas = this.ui.canvas[0],
        ...
        scaled = Utils.scale(-dBFS, 48, 0, 0, height),
        now = Date.now(),
